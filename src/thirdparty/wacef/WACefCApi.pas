...
type
  ...
  //............................................................................cef_base_capi
	TCefBase=record
		...
		add_ref:function(self:PCefBase): cint; {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
		// Decrement the reference count.  Delete this object when no references
		// remain.
		release:function(self:PCefBase): cint; {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
		// Returns the current number of references.
		get_refct:function(self:PCefBase): cint; {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
	end;
  ...
  //............................................................................cef_app_capi
	...
	TCefBrowserHost=record
		...
		show_dev_tools:procedure(self:PCefBrowserHost; const windowInfo:PCefWindowInfo; client:PCefClient; const settings:PCefBrowserSettings); {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
		...
		// cef_render_handler_t::OnPaint asynchronously with the updated regions. This
		// function is only used when window rendering is disabled.
		invalidate:procedure(self:PCefBrowserHost; const dirtyRect:PCefRect; _type:TCefPaintElementType); {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
  ...
  TCefCommandLine = record
    ...
    init_from_argv: procedure (self: PCefCommandLine; argc: CInt; const argv: PPCChar);{$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
	...
	TCefDomNode=record
		...
		add_event_listener:procedure(self:PCefDomnode; const eventType:PCefString; listener:PCefDomeventListener; useCapture: cint); {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
	...
	TCefDomEvent=record
		...
		// The resulting string must be freed by calling cef_string_userfree_free().
		get_type:function(self:PCefDomevent):PCefStringUserFree; {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
		// Returns the event category.
		get_category:function(self:PCefDomevent):TCefDomEventCategory; {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
		// Returns the event processing phase.
		get_phase:function(self:PCefDomevent):TCefDomEventPhase; {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
		// Returns true (1) if the event can bubble up the tree.
		can_bubble:function(self:PCefDomevent): cint; {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
		// Returns true (1) if the event can be canceled.
		can_cancel:function(self:PCefDomevent): cint; {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
		// Returns the document associated with this event.
		get_document:function(self:PCefDomevent):PCefDomdocument; {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
		// Returns the target of the event.
		get_target:function(self:PCefDomevent):PCefDomnode; {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
		// Returns the current target of the event.
		get_current_target:function(self:PCefDomevent):PCefDomnode; {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
	end;
	...
	TCefDomeventListener=record
		...
		// this function.
		handle_event:procedure(self:PCefDomeventListener; event:PCefDomevent); {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
	end;
	...
	TCefGeolocationHandler=record
		...
		// request.
		on_request_geolocation_permission:procedure(self:PCefGeolocationHandler; browser:PCefBrowser; const requesting_url:PCefString; request_id: cint; callback:PCefGeolocationCallback); {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
  ...
  //............................................................................cef_print_handler_capi
	...
	TCefRenderHandler=record
		...
		// drawn correctly.
		get_screen_info:function(self:PCefRenderHandler; browser:PCefBrowser; out screen_info:PCefScreenInfo): cint; {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
		...
		on_paint:procedure(self:PCefRenderHandler; browser:PCefBrowser; _type:TCefPaintElementType; dirtyRectsCount: csize_t; const dirtyRects:PCefRectArray; const buffer:Pointer; width: cint; height: cint); {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
	...
	TCefPostDataElement=record
		...
		// copied.
		set_to_bytes:procedure(self:PCefPostDataElement; size: csize_t; const bytes:Pointer); {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
		...
		// actually read.
		get_bytes:function(self:PCefPostDataElement; size: csize_t; bytes:Pointer): csize_t; {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
	end;
	...
	TCefResourceBundleHandler=record
		...
		get_data_resource:function(self:PCefResourceBundleHandler; resource_id: cint; var data:Pointer; var data_size: csize_t): cint; {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
	end;
	...
	TCefResourceHandler=record
		...
		read_response:function(self:PCefResourceHandler; data_out:Pointer; bytes_to_read: cint; bytes_read: pcint; callback:PCefCallback): cint; {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
	...
	TCefReadHandler=record
		...
		read:function(self:PCefReadHandler; ptr:Pointer; size: csize_t; n: csize_t): csize_t; {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
	...
	TCefStreamReader=record
		...
		base:TCefBase;
		// Read raw binary data.
		read:function(self:PCefStreamReader; ptr:Pointer; size: csize_t; n: csize_t): csize_t; {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
		// Seek to the specified offset position. |whence| may be any one of SEEK_CUR,
	...
	TCefWriteHandler=record
		...
		write:function(self:PCefWriteHandler; const ptr:Pointer; size: csize_t; n: csize_t): csize_t; {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
		// Seek to the specified offset position. |whence| may be any one of SEEK_CUR,
		// SEEK_END or SEEK_SET. Return zero on success and non-zero on failure.
	...
	TCefStreamWriter=record
		...
		base:TCefBase;
		// Write raw binary data.
		write:function(self:PCefStreamWriter; const ptr:Pointer; size: csize_t; n: csize_t): csize_t; {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
		// Seek to the specified offset position. |whence| may be any one of SEEK_CUR,
		// SEEK_END or SEEK_SET. Returns zero on success and non-zero on failure.
	...
	TCefUrlrequestClient=record
		...
		on_download_data:procedure(self:PCefUrlrequestClient; request:PCefUrlrequest; const data:Pointer; data_length: csize_t); {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
		// Called on the IO thread when the browser needs credentials from the user.
		// |isProxy| indicates whether the host is a proxy server. |host| contains the
		// hostname and |port| contains the port number. Return true (1) to continue
		// the request and call cef_auth_callback_t::cont() when the authentication
	...
	TCefV8handler=record
		...
		execute:function(self:PCefV8handler; const name:PCefString; _object:PCefV8value; argumentsCount: csize_t; const arguments:PPCefV8value; var retval:PCefV8value; var exception:TCefString): cint; {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
	end;
	...
	TCefBinaryValue=record
		...
		get_data:function(self:PCefBinaryValue; buffer:Pointer; buffer_size: csize_t; data_offset: csize_t): csize_t; {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
	end;
	...
	TCefZipReader=record
		...
		read_file:function(self:PCefZipReader; buffer:Pointer; bufferSize: csize_t): cint; {$IFNDEF UNIX}stdcall{$ELSE}cdecl{$ENDIF};
